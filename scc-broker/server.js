...
const SCC_STATE_SERVER_HOST = "state" || "localhost" || "18.141.187.151";
const SCC_STATE_SERVER_PORT = Number(process.env.SCC_STATE_SERVER_PORT) || 7777;
const SCC_INSTANCE_IP = process.env.SCC_INSTANCE_IP || "broker" || "localhost";
...
const STATE_SERVER_CONNECT_TIMEOUT =
	Number(process.env.SCC_STATE_SERVER_CONNECT_TIMEOUT) || 3000;
const STATE_SERVER_ACK_TIMEOUT =
	Number(process.env.SCC_STATE_SERVER_ACK_TIMEOUT) || 2000;
const BROKER_SERVER_CONNECT_TIMEOUT =
	Number(process.env.SCC_BROKER_SERVER_CONNECT_TIMEOUT) || 10000;
const BROKER_SERVER_ACK_TIMEOUT =
	Number(process.env.SCC_BROKER_SERVER_ACK_TIMEOUT) || 10000;
			...
			for await (let action of middlewareStream) {
				if (action.type === Action.HANDSHAKE_WS) {
					...
					if (!urlParts.query || urlParts.query.authKey !== SCC_AUTH_KEY) {
						let err = new Error(
							"Cannot connect to the cluster broker server without providing a valid authKey as a URL query argument."
						);
		...
		async (middlewareStream) => {
			for await (let action of middlewareStream) {
				if (action.type === Action.SUBSCRIBE) {
					console.log(
						`${action.socket.remoteAddress} subscribed to ${action.channel}`
					);
				} else if (action.type === Action.PUBLISH_IN) {
					if (LOG_LEVEL >= 3) {
						console.log(
							`${action.socket.remoteAddress} published to ${action.channel}`
						);
					}
				}
