...
const DEFAULT_CLUSTER_STARTUP_DELAY = 0; //was 5000 by default (waiting for broker instances before workers)
...
const CLUSTER_SCALE_OUT_DELAY = selectNumericArgument([
	process.env.SCC_STATE_SERVER_SCALE_OUT_DELAY,
	DEFAULT_CLUSTER_SCALE_OUT_DELAY,
]);
const CLUSTER_SCALE_BACK_DELAY = selectNumericArgument([
	process.env.SCC_STATE_SERVER_SCALE_BACK_DELAY,
	DEFAULT_CLUSTER_SCALE_BACK_DELAY,
]);
const STARTUP_DELAY = selectNumericArgument([
	process.env.SCC_STATE_SERVER_STARTUP_DELAY,
	DEFAULT_CLUSTER_STARTUP_DELAY,
]);
...
if (!serverReady) {
	logInfo(
		`Waiting ${STARTUP_DELAY}ms for initial scc-broker instances before allowing scc-worker instances to join`
	);
	setTimeout(function () {
		logInfo(
...
let sccBrokerLeaveCluster = function (socket, req) {
	...
	setBrokerClusterScaleTimeout(() => {
		invokeRPCOnAllInstances(
			sccWorkerSockets,
			"sccBrokerLeaveCluster",
	...
	logInfo(
		`The scc-broker instance ${socket.instanceId} at address ${socket.instanceIp} on port ${socket.instancePort} left the cluster on socket ${socket.id}`
	);
};
...
let sccWorkerLeaveCluster = function (socket, req) {
	...
	setWorkerClusterScaleTimeout(() => {
		invokeRPCOnAllInstances(
			sccWorkerSockets,
			"sccWorkerLeaveCluster",
			getSCCWorkerClusterState()
		);
	}, CLUSTER_SCALE_BACK_DELAY);
	...
	logInfo(
		`The scc-worker instance ${socket.instanceId} at address ${socket.instanceIp} left the cluster on socket ${socket.id}`
	);
};
...
let invokeRPCOnInstance = async function (socket, procedureName, data) {
	...
	try {
		...
		if (socket.state === "open") {
			setTimeout(
				invokeRPCOnInstance.bind(null, socket, procedureName, data),
				RETRY_DELAY
			);
		}
	}
...
let getRemoteIp = function (socket, data) {
	let forwardedAddress = FORWARDED_FOR_HEADER
		? (socket.request.headers[FORWARDED_FOR_HEADER] || "").split(",")[0]
		: null;
	...
	async (middlewareStream) => {
		for await (let action of middlewareStream) {
			if (action.type === Action.HANDSHAKE_WS) {
				if (SCC_AUTH_KEY) {
					...
					if (!urlParts.query || urlParts.query.authKey !== SCC_AUTH_KEY) {
						let err = new Error(
							"Cannot connect to the scc-state instance without providing a valid authKey as a URL query argument."
						);
			...
			if (action.type === Action.HANDSHAKE_SC) {
				...
				let sccComponentIsObsolete =
					!instanceType || Number.isNaN(reportedMajorSemver);

				if (reportedMajorSemver !== requiredMajorSemver) {
					let err;
					if (sccComponentIsObsolete) {
						err = new Error(
							`An obsolete SCC component at address ${remoteAddress} is incompatible with the scc-state@^${packageVersion}. Please, update the SCC component up to version ^${requiredMajorSemver}.0.0`
						);
					} else if (reportedMajorSemver > requiredMajorSemver) {
						err = new Error(
							`The scc-state@${packageVersion} is incompatible with the ${instanceType}@${version}. Please, update the scc-state up to version ^${reportedMajorSemver}.0.0`
						);
					} else {
						err = new Error(
							`The ${instanceType}@${version} at address ${remoteAddress}:${instancePort} is incompatible with the scc-state@^${packageVersion}. Please, update the ${instanceType} up to version ^${requiredMajorSemver}.0.0`
						);
					}
	...
	for await (let { socket } of agServer.listener("connection")) {
		(async () => {
			for await (let req of socket.procedure("sccBrokerJoinCluster")) {
				...
				req.end();
				logInfo(
					`The scc-broker instance ${data.instanceId} at address ${socket.instanceIp} on port ${socket.instancePort} joined the cluster on socket ${socket.id}`
				);
			}
		})();
		...
		(async () => {
			for await (let req of socket.procedure("sccWorkerJoinCluster")) {
				...
				if (!serverReady) {
					logWarning(
						`The scc-worker instance ${data.instanceId} at address ${socket.instanceIp} on socket ${socketId} was not allowed to join the cluster because the server is waiting for initial brokers`
					);
					req.error(
				...
				logInfo(
					`The scc-worker instance ${data.instanceId} at address ${socket.instanceIp} joined the cluster on socket ${socketId}`
				);
			}
		})();
